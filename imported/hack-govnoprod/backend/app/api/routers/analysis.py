        ...
        for question in pipeline_state.clarify_questions:
            api_questions.append(
                ClarifyQuestion(
                    ...
                    priority=getattr(
                        question, "priority", getattr(question, "importance", "medium")
                    ),
                )
            )
...
async def download_report(prompt_id: str):
    ...
    return JSONResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=analysis-{prompt_id}.json"
        },
    )
...
def _export_as_markdown(report, patches: list[Patch]) -> str:
    ...
    lines.append(
        f"- **Length:** {report.length_words} words, {report.length_chars} characters"
    )
    ...
    if report.contradictions:
        ...
        for i, contradiction in enumerate(report.contradictions, 1):
            lines.append(
                f"{i}. **{contradiction.severity.upper()}:** {contradiction.description}"
            )
        lines.append("")
    ...
    if patches:
        ...
        if safe_patches:
            lines.append("### Safe Improvements")
            lines.append("")
        ...
        if risky_patches:
            lines.append("### Advanced Improvements (Review Required)")
            lines.append("")
    ...
    # Generated by
    lines.append("---")
    lines.append("*Generated by Curestry AI Prompt Analysis System*")
...
def _export_as_xml(report, patches: list[Patch]) -> str:
    ...
    lines.append(
        f"    <semantic-entropy>{report.semantic_entropy.entropy:.3f}</semantic-entropy>"
    )
    ...
    if report.contradictions:
        ...
        for contradiction in report.contradictions:
            lines.append(
                f'    <contradiction type="{contradiction.type}" severity="{contradiction.severity}">'
            )
            lines.append(
                f"      <description><![CDATA[{contradiction.description}]]></description>"
            )
    ...
    if patches:
        ...
        for patch in patches:
            lines.append(
                f'    <patch id="{patch.id}" type="{patch.type}" category="{patch.category}">'
            )
            lines.append(
                f"      <description><![CDATA[{patch.description}]]></description>"
            )
